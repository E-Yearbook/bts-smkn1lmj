...
<div class="mb-8 flex items-center justify-between">
    <div>
        ...
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Manage all books
            @if (!$books->isEmpty())
                <span class="ml-2 inline-block rounded-full bg-brand-100 dark:bg-brand-900/30 px-2.5 py-0.5 text-xs font-medium text-brand-700 dark:text-brand-300">
    ...
    <a href="{{ route('books.create') }}"
        class="inline-flex items-center gap-2 rounded-lg bg-brand-500 px-5 py-2.5 text-sm font-medium text-white shadow-theme-xs">
        ...
        <table class="w-full text-sm text-left">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                ...
                <tr class="bg-white dark:bg-gray-900">
                    <td class="px-4 py-3 text-gray-500 dark:text-gray-300">{{ $i + 1 }}</td>
                    <td class="px-4 py-3">
                        @if ($book->book_cover)
                    ...
                    <td class="px-4 py-3 text-gray-600 dark:text-white">{{ $book->publisher }}</td>
                    <td class="px-4 py-3">
                    ...
                    <td class="px-4 py-3 text-gray-600 dark:text-white">{{ $book->yearCover->year ?? '-' }}</td>
                    <td class="px-4 py-3 text-gray-600 dark:text-white">{{ $book->user->name ?? '-' }}</td>
                    <td class="px-4 py-3">
                        <div class="flex items-center justify-center gap-2">
                            <a href="{{ route('books.show', $book->id) }}"
                                class="inline-flex items-center gap-1 rounded-lg bg-brand-50 px-3 py-1.5 text-xs font-medium text-brand-500 hover:bg-brand-100 dark:bg-brand-950 dark:hover:bg-brand-900 transition-colors">
                            ...
                            <a href="{{ route('books.edit', $book->id) }}"
                                class="inline-flex items-center gap-1 rounded-lg bg-warning-50 px-3 py-1.5 text-xs font-medium text-warning-500 hover:bg-warning-100 dark:bg-warning-950 dark:hover:bg-warning-900 transition-colors">
                                <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            ...
                            <button onclick="confirmDelete({{ $book->id }}, '{{ addslashes($book->name) }}')"
                                class="inline-flex items-center gap-1 rounded-lg bg-error-50 px-3 py-1.5 text-xs font-medium text-error-500 hover:bg-error-100 dark:bg-error-950 dark:hover:bg-error-900 transition-colors">
                                <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
