...
<style>
    .dropzone { border: 2px dashed #d1d5db; border-radius: 12px; background: #f9fafb; min-height: 200px; padding: 20px; transition: all 0.3s ease; cursor: pointer; }
    .dropzone.dz-drag-hover { border-color: #3b82f6; background: #eff6ff; box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.1); transform: scale(1.01); }
    .dropzone .dz-message { margin: 0; }
    .dropzone .dz-preview { margin: 0; position: relative; }
    .dropzone .dz-preview.dz-file-preview { background: transparent; border: none; padding: 0; margin: 0; }
    .dropzone .dz-details { display: none; }
    .dropzone .dz-remove { position: absolute; top: 8px; right: 8px; opacity: 0; transition: opacity 0.2s ease; z-index: 10; }
    .dropzone .dz-preview:hover .dz-remove { opacity: 1; }
    .file-preview-container { display: flex; flex-direction: column; align-items: center; gap: 16px; padding: 12px; }
    .file-preview-img { max-width: 100%; max-height: 140px; border-radius: 8px; border: 1px solid #e5e7eb; object-fit: contain; background: white; padding: 4px; }
    .file-info { text-align: center; width: 100%; }
    .file-name { font-size: 0.875rem; font-weight: 500; color: #1f2937; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin-bottom: 4px; }
    .file-meta { font-size: 0.75rem; color: #6b7280; display: flex; justify-content: center; gap: 8px; flex-wrap: wrap; }
    .progress-bar-wrapper { width: 100%; height: 4px; background: #e5e7eb; border-radius: 2px; overflow: hidden; margin-top: 8px; display: none; }
    .progress-bar-wrapper.show { display: block; }
    .progress-bar { height: 100%; background: linear-gradient(90deg, #3b82f6, #1d4ed8); width: 0%; transition: width 0.3s ease; }
</style>
@endpush
...
<div class="mb-8 flex items-center justify-between">
    <div>
        <h1 class="text-2xl font-semibold text-gray-900">Add Cover</h1>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div class="space-y-6">
            <div class="rounded-2xl border border-gray-200 bg-white p-6">
                <h3 class="mb-4 text-base font-semibold text-gray-800">Informasi Cover</h3>
                <div class="space-y-4">
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-gray-700">
                            Year <span class="text-error-500">*</span>
                        </label>
                        <input type="number" name="year" id="year" value="{{ old('year') }}" min="2000" max="2100" placeholder="Contoh: 2024"
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-6">
            ...
            <div id="coverDropzone" class="dropzone rounded-xl">
                <div class="dz-message needsclick">
                    ...
                    <p class="text-sm font-semibold text-gray-700 mb-1">Drag & Drop file</p>
...
const myDropzone = new Dropzone('#coverDropzone', {
    ...
    acceptedFiles: 'image/jpeg,image/png', addRemoveLinks: false, dictDefaultMessage: '',
    ...
    init: function () {
        ...
        this.on('error', (file, message) => { Swal.fire({ icon: 'error', title: 'Oops!', text: message, confirmButtonColor: '#465fff' }); this.removeFile(file); });
        this.on('uploadprogress', (file, progress) => {
            if (file.previewElement) { file.previewElement.querySelector('.progress-bar-wrapper').classList.add('show'); file.previewElement.querySelector('.progress-bar').style.width = progress + '%'; }
        });
        this.on('addedfile', (file) => {
            const removeBtn = file.previewElement.querySelector('.dz-remove');
            if (removeBtn) removeBtn.addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); this.removeFile(file); });
        });
    }
});
