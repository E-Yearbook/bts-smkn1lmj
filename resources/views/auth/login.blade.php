...
<body
    ...
    <div class="relative bg-white z-1 dark:bg-gray-900">
        <div class="flex w-full h-screen overflow-hidden">
            ...
            <div class="flex flex-col flex-1 w-full lg:w-1/2 overflow-y-auto">
                ...
                <div class="flex flex-col justify-center flex-1 w-full max-w-md mx-auto px-6">
                    <div>
                        ...
                        <div x-data="{
                            loading: false,
                            seconds: {{ session('seconds_left', 0) }},
                            init() {
                                if (this.seconds > 0) {
                            ...
                            <form method="POST" action="{{ route('login-post') }}"
                                @submit="if(seconds > 0) { $event.preventDefault(); return false; } loading = true">
                                @csrf
                                <div class="space-y-5">
                                    ...
                                    <div>
                                        ...
                                        <input type="email" name="email" required
                                            class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 dark:border-gray-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-brand-500" />
                                    </div>
                                        ...
                                        <button type="submit"
                                            ...
                                            @click="if(seconds > 0) {
                                                Swal.fire({
                                                    ...
                                                    text: 'Terlalu banyak percobaan. Silakan coba lagi dalam ' + seconds + ' detik.',
                                                    confirmButtonColor: '#4F46E5'
                                                });
                                            }"
                                            :class="loading || seconds > 0
                                            ...
                                            <template x-if="seconds > 0">
                                                <span x-text="'Tunggu (' + seconds + 's)'"></span>
                                            </template>

                                            <template x-if="loading && seconds <= 0">
                                                <div class="flex items-center">
                                                    <svg class="w-5 h-5 mr-3 animate-spin text-white"
            ...
            <!-- ===== Foto Kanan ===== -->
            <div class="hidden lg:block lg:w-1/2 h-screen flex-shrink-0">
                <img
